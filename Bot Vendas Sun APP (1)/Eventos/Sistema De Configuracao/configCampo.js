...
**Deseja adicionar o valor de \`${qtdlinhas}\`\ itens ao estoque de \`${ggg22.campo}\`?**
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'estoquearquivo') {
                ...
                interaction.reply({ 
                    ephemeral: true, 
                    content: `❓ Envie um ou mais arquivo de texto contendo o estoque que deseja adicionar, expira <t:${Math.ceil(timeStamp / 1000)}:R>.`
                }).then(msgggg => {
                    ...
                    collector.on('collect', async message => {
                        if (message.attachments.size > 0) {
                            ...
                            try {
                                ...
                                const linhasNaoVazias = lines.filter(line => line.trim());
                                ...
                                await interaction.editReply({
                                    content: `Total de \`${linhasNaoVazias.length}\` itens detectados, cada item será adicionado como um produto no estoque de \`${ggg.camposelect}\`, exemplo:\n\`\`\`${linhasNaoVazias[0]}\`\`\`\nEsse valor será entregue como **uma** unidade para o cliente.\n**Deseja adicionar o valor de \`${linhasNaoVazias.length}\` itens ao estoque de \`${ggg.camposelect}\`?**`,
                                    components: [row],
                                    ephemeral: true
                                });
                                ...
                                if (error.response && error.response.status === 404) {
                                    await interaction.editReply({ content: '❌ Ocorreu um erro ao processar o arquivo: URL do anexo não encontrada. Tente novamente.' });
                                } else {
                                    await interaction.editReply({ content: '❌ Ocorreu um erro ao processar o arquivo. Tente novamente.' });
                                }
                            }
                        }
                    });
